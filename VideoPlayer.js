...
export default class VideoPlayer extends Component {
  ...
  constrainToSeekerMinMax(val = 0) {
    ...
    if (val <= 0) {
      ...
      console.log("this.player.seekerWidth", this.player.seekerWidth)
  ...
  initSeekPanResponder() {
    this.player.seekPanResponder = PanResponder.create({
      ...
      onPanResponderGrant: (evt, gestureState) => {
        let state = this.state;
        this.clearControlTimeout();
        let position = evt.nativeEvent[I18nManager.isRTL ? 'pageX' : 'locationX'];
        if (I18nManager.isRTL) {
          position = (width - 40) - position
        }
        this.setSeekerPosition(position);
        state.seeking = true;
      ...
      onPanResponderMove: (evt, gestureState) => {
        const position = this.state.seekerOffset + gestureState.dx * (I18nManager.isRTL ? -1 : 1);
        this.setSeekerPosition(position);
        let state = this.state;
